...
class Goal(Base):
    ...
    def _get_base_goal(self) -> 'Goal':
        ...
        try:
            base_goal = [
                x for x in base_hazard.goals if x.metric.name == self.metric.name][0]
            return base_goal
        except IndexError as err:
            logging.exception(
                'Unable to find base goal for metric %s.', self.metric.name)
...
class Hazard(Base):
    ...
    goals: List[Goal] = relationship(
        'Goal', order_by=Goal.id, back_populates='hazard', cascade='delete')
...
class Project(Base):
    ...
    hazards: List[Hazard] = relationship(
        'Hazard', order_by=Hazard.name, back_populates='project', cascade='delete')
    ...
    def _save_system_db_as(self, path: str):
        shutil.copy2(f'{self.dir}/{CURRENT_SYSTEM_DB_PATH}',
                     f'{self.dir}/{path}')

    def _load_system_db_from(self, path: str):
        try:
            os.replace(f'{self.dir}/{path}',
                       f'{self.dir}/{CURRENT_SYSTEM_DB_PATH}')
    ...
    def update_object_parameter(self, spine_db: SpineDBSession, obj_id: int, param_name: str, val: str):
        spine_db.update_object_parameter(
            self.dir, CURRENT_SYSTEM_DB_PATH, obj_id, param_name, val)

    def update_relationship_object(self, spine_db: SpineDBSession, rel_id: int, obj_index: int, obj_name: str):
        spine_db.update_relationship_object(
            self.dir, CURRENT_SYSTEM_DB_PATH, rel_id, obj_index, obj_name)
    ...
    def import_metrics(self, session: DBSession, spine_db: SpineDBSession) -> List[Metric]:
        ...
        metrics = [Metric(name=o.object_name)
                   for o in objects if o.object_class_name == 'metrics']
        for m in metrics:
            session.add(m)
        return metrics
    ...
    def update_goal(self, session: DBSession, hazard_name: str, metric_name: str, comparison: str, target_value: float):
        try:
            ...
            logging.exception(
                'Goal with metric %s for hazard %s not found.', metric_name, hazard_name)
            logging.exception(err)
            return
    ...
    def load_results(self, spine_db: SpineDBSession, baseline=False) -> List[Hazard]:
        ...
        for r in relationships:
            ...
            metric = [g.metric for g in hazard.goals if g.metric.name ==
                      r.objects[1].object_name][0]
            ...
            try:
                ...
                logging.exception('Skipping result %s because value is not a float (%s)', r.name, str(
    ...
    def calculate_proposed_changes(self, spine_db: SpineDBSession) -> Tuple[List[SpineObject], List[SpineObject], List[SpineObject]]:
        proposed_objects = spine_db.get_objects(
            self.dir, CURRENT_SYSTEM_DB_PATH)
        proposed_names = [obj.object_name for obj in proposed_objects]
        baseline_objects = spine_db.get_objects(
            self.dir, BASELINE_SYSTEM_DB_PATH)
        baseline_names = [obj.object_name for obj in baseline_objects]

        added = [
            obj for obj in proposed_objects if obj.object_name not in baseline_names]
        removed = [
            obj for obj in baseline_objects if obj.object_name not in proposed_names]

        changed = []
        for obj in proposed_objects:
            if obj.object_name in baseline_names:
                baseline_obj = [
                    x for x in baseline_objects if x.object_name == obj.object_name][0]
                new_obj = SpineObject(
                    obj.object_class_name, obj.object_class_id, obj.object_id, obj.object_name)
